...
```{r echo = FALSE, out.width="100%"}
ggplot(blomkvist, aes(x = smoker, 
                      y = 0, 
                      colour = rt_hand_d)) +
  geom_jitter(width = .25, height = .5, size = 3, alpha = .75) +
...
ggplot(blomkvist, 
       aes(x = smoker, 
           y = 0, 
           size = rt_hand_d)) +
  geom_jitter(width = .25, height = .5, alpha = .25) +
  scale_y_continuous(limits = c(-.75,.75)) +
  theme(axis.text.y = element_blank(),
        axis.ticks.y = element_blank(),
        axis.title.y = element_blank())
...
- **exercises/3c_aesthetics_and_attributes**
...
- more `geoms` in other packages such as `tidybayes` and `ggridges`
- choice depends on visualisation goal (and your subject domain)
- many can be combined
- different obligatory aesthetics
...
- examples: (jitter) dots, box plot, violin plot, barplot (pie chart), dynamite plots
