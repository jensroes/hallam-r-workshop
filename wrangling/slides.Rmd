...
```{r}
blomkvist_id_age <- select(blomkvist, id, age)
blomkvist_id_age
```
...
```{r}
blomkvist_med_smoke <- select(blomkvist, medicine, smoker)
blomkvist_med_smoke
```
...
```{r}
# Combine two datasets side-by-side
bind_cols(blomkvist_id_age, blomkvist_med_smoke)
```
...
```{r}
blomkvist_former_smokers <- filter(blomkvist, smoker == "former")
blomkvist_smokers <- filter(blomkvist, smoker == "yes")
```
...
```{r}
# Stacking two data sets
blomkvist_smoking <- bind_rows(blomkvist_former_smokers, blomkvist_smokers)
```
...
```{r}
slice_head(blomkvist_smoking, n = 2)
```

```{r}
slice_tail(blomkvist_smoking, n = 2)
```
...
```{r}
blomkvist_smoker <- select(blomkvist, smoker, age)
blomkvist_smoker_o40 <- filter(blomkvist_smoker, age > 40)
```

```{r}
blomkvist_sex <- select(blomkvist, sex, age)
blomkvist_sex_u50 <- filter(blomkvist_sex, age < 50)
```

## Create two example data sets {.columns-2}

```{r}
blomkvist_smoker_o40
```
...
```{r}
# Keep all rows of blomkvist_sex_u50
left_join(blomkvist_sex_u50, blomkvist_smoker_o40, by = "age")
```

## Combine data: `right_join()`

```{r}
# Keep all rows of blomkvist_smoker_o40
right_join(blomkvist_sex_u50, blomkvist_smoker_o40, by = "age")
```
...
```{r}
# Include all rows of both data sets
full_join(blomkvist_sex_u50, blomkvist_smoker_o40, by = "age")
```
## Combine data: `inner_join()`

```{r}
# Include only info that is present in both data sets
inner_join(blomkvist_sex_u50, blomkvist_smoker_o40, by = "age")
```
...
```{r}
# (Almost) everything we did before in one go.
read_csv("../data/blomkvist.csv") %>% select(id, starts_with("rt_")) %>%
  pivot_longer(-id, names_to = c(".value", "response_by", "dominant"), 
                    names_pattern = "(.+)_(.+)_(.+)") %>%
  filter(rt < quantile(rt, probs = .975, na.rm = TRUE)) %>% 
  mutate(across(dominant, recode, d = "dominant", nd = "non-dominant")) %>%
  group_by(response_by, dominant) %>%
  summarise(across(rt, list(mean = mean, median = median, sd = sd), na.rm = TRUE)) %>% 
  ungroup()
```
...
```{r fig.align='right', out.width="70%", echo = F}
